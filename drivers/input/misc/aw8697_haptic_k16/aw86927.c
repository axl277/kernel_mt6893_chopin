					     ...
					     unsigned int cnt)
{
	...
	while (cnt) {
		...
		if (!ret) {
			pr_info("%s: entered standby!\n", __func__);
			break;
		}
		cnt--;
		pr_info("%s: wait for standby\n", __func__);
...
static int aw86927_set_base_addr(struct aw86927 *aw86927)
{
	...
	if (!aw86927->ram.base_addr) {
		pr_err("%s:aw86927 ram base addr is error\n", __func__);
		return ret;
	}
	/* rtp */
...
static int aw86927_get_irq_state(struct aw86927 *aw86927)
{
	...
	if (reg_val & AW86927_BIT_SYSINT_FF_AEI) {
		pr_info("%s aw86927 rtp fifo almost empty\n", __func__);
		ret |= AW86927_SYSINT_FF_AEI;
	}
	if (reg_val & AW86927_BIT_SYSINT_FF_AFI) {
		pr_info("%s aw86927 rtp fifo almost full\n", __func__);
...
static int aw86927_haptic_cont_get_f0(struct aw86927 *aw86927)
{
	...
	if (!aw86927->info.cont_track_margin) {
		pr_err("%s aw86927->info.cont_track_margin = 0!\n",
			   __func__);
	} else {
...
static int aw86927_haptic_get_vbat(struct aw86927 *aw86927)
{
	...
	pr_info("%s aw86927->vbat=%dmV, vbat_code=0x%02X\n",
					   ...
					   unsigned char mode)
{
	...
	case AW86927_BST_MODE_BYPASS:
		pr_info("%s haptic bst mode = bypass\n",
			    __func__);
		aw86927_i2c_write_bits(aw86927, AW86927_REG_PLAYCFG1,
				       AW86927_BIT_PLAYCFG1_BST_MODE_MASK,
...
static int aw86927_haptic_get_ram_number(struct aw86927 *aw86927)
{
	...
	pr_info("%s enter!\n", __func__);
	if (!aw86927->ram_init) {
		pr_err("%s: ram init faild, ram_num = 0!\n", __func__);
		return -EPERM;
	}
	...
	pr_info("%s: ram_version = 0x%02x\n", __func__, ram_data[0]);
	pr_info("%s: first waveform addr = 0x%04x\n", __func__,
		first_wave_addr);
...
static int aw86927_rtp_osc_calibration(struct aw86927 *aw86927)
{
	...
	while (1) {
		...
		if (osc_int_state & AW86927_BIT_SYSINT_DONEI) {
			...
			pr_info("%s osc trim playback done aw86927->rtp_cnt= %d\n",
		...
		if (aw86927->microsecond > OSC_CALIBRATION_T_LENGTH) {
			pr_info("%s osc trim time out! aw86927->rtp_cnt %d osc_int_state %02x\n",
	...
	aw86927->microsecond = ktime_to_us(ktime_sub(aw86927->kend,
						aw86927->kstart));
	/*calibration osc */
	pr_info("%s aw86927_microsecond: %ld\n", __func__,
		aw86927->microsecond);
	pr_info("%s exit\n", __func__);
	return 0;
}
					...
					unsigned long int real_time)
{
	...
	if (theory_time == real_time) {
		pr_info("%s theory_time == real_time: %ld, no need to calibrate!\n",
			__func__, real_time);
		return 0;
	} else if (theory_time < real_time) {
		if ((real_time - theory_time) > (theory_time / 50)) {
			pr_info("%s (real_time - theory_time) > (theory_time/50), can't calibrate!\n",
				__func__);
		...
		if ((real_time - theory_time) <
			...
			pr_info("%s real_time: %ld, theory_time: %ld, no need to calibrate!\n",
		...
		if ((theory_time - real_time) > (theory_time / 50)) {
			pr_info("%s (theory_time - real_time) > (theory_time / 50), can't calibrate!\n",
				__func__);
			return DFT_LRA_TRIM_CODE;
		}
		if ((theory_time - real_time) <
...
static void aw86927_calculate_cali_step(struct aw86927 *aw86927)
{
	...
	pr_info("%s origin trim_lra = 0x%02X, f0_cali_lra = 0x%02X, final f0_calib_data = 0x%02X\n",
...
static int aw86927_haptic_is_within_cali_range(struct aw86927 *aw86927)
{
	...
	pr_info("%s f0_pre = %d, f0_cali_min = %d, f0_cali_max = %d, f0 = %d\n",
				     ...
				     const char *buf, size_t count)
{
	...
	pr_debug("%swrite szie %zd, period size %lu", __func__, count,
		 period_size);
				  ...
				  const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%d %d %d %d %d %d %d %d %d", &databuf[0], &databuf[1],
		...
		if (databuf[0] < 1 || databuf[0] > 3) {
			pr_info("%s: input trig_num out of range!\n",
				    __func__);
			return count;
		}
		if (!aw86927->ram_init) {
		...
		if (databuf[4] > aw86927->ram.ram_num ||
			...
			pr_err("%s: input seq value out of range!\n",
				__func__);
			return count;
		}
		databuf[0] -= 1;
		     ...
		     struct device_node *np)
{
	...
	val = of_property_read_u32(np, "aw86927_vib_brk_bst_md",
				   &aw86927->info.brk_bst_md);
	if (val != 0)
		pr_info("%s vib_brk_bst_md not found\n", __func__);
	...
	for_each_available_child_of_node(np, child_node) {
		...
		rc = of_property_read_u32(child_node, "mtk,effect-id",
					  &effect->id);
		if (rc != 0)
			pr_info("%s Read mtk,effect-id failed\n", __func__);

		effect->vmax_mv = config->vmax_mv;
		rc = of_property_read_u32(child_node, "mtk,wf-vmax-mv", &tmp);
		if (rc != 0)
			pr_info("%s  Read mtk,wf-vmax-mv failed !\n", __func__);
		else
			effect->vmax_mv = tmp;

		pr_info("%s ---%d effect->vmax_mv =%d\n", __func__, __LINE__,
			effect->vmax_mv);
		rc = of_property_count_elems_of_size(child_node,
						     "mtk,wf-pattern",
						     sizeof(u8));
		if (rc < 0) {
			pr_info("%s Count mtk,wf-pattern property failed !\n",
			       __func__);
		} else if (rc == 0) {
			pr_info("%s mtk,wf-pattern has no data\n", __func__);
		}
		pr_info("%s ---%d\n", __func__, __LINE__);
		...
		rc = of_property_read_u8_array(child_node, "mtk,wf-pattern",
					       ...
					       effect->pattern_length);
		if (rc < 0) {
			pr_info("%s Read mtk,wf-pattern property failed !\n",
			       __func__);
		}
		...
		rc = of_property_read_u32(child_node, "mtk,wf-play-rate-us",
					  &tmp);
		if (rc < 0)
			pr_info("%s Read mtk,wf-play-rate-us failed !\n",
			       __func__);
		else
		...
		rc = of_property_read_u32(child_node, "mtk,wf-repeat-count",
					  &tmp);
		if (rc < 0) {
			pr_info("%s Read  mtk,wf-repeat-count failed !\n",
			       __func__);
		} else {
		...
		rc = of_property_read_u32(child_node, "mtk,wf-s-repeat-count",
					  &tmp);
		if (rc < 0) {
			pr_info("%s Read  mtk,wf-s-repeat-count failed !\n",
			       __func__);
		} else {
		...
		if (tmp > HAP_BRAKE_PATTERN_MAX) {
			pr_info("%s wf-brake-pattern shouldn't be more than %d bytes\n",
		...
		rc = of_property_read_u8_array(child_node,
					       ...
					       effect->brake, tmp);
		if (rc < 0) {
			pr_info("%s Failed to get wf-brake-pattern !\n",
			       __func__);
		}
	...
	pr_info("%s aw86927->info.brk_bst_md: %d\n",
		    __func__, aw86927->info.brk_bst_md);
	pr_info("%s aw86927->info.bst_vol_default: %d\n",
		    __func__, aw86927->info.bst_vol_default);
	pr_info("%s aw86927->info.bst_vol_ram: %d\n",
		    __func__, aw86927->info.bst_vol_ram);
	pr_info("%s aw86927->info.bst_vol_rtp: %d\n",
...
{
	pr_debug("%s enter\n", __func__);
	pr_debug("%s: aw86927->play.vmax_mv =0x%x\n", __func__,
	...
	if (aw86927->state) {
		...
		if (aw86927->is_custom_wave == 0) {
			aw86927->rtp_file_num = aw86927->effect_id -
					aw86927->info.effect_id_boundary;
			pr_info("%s: aw86927->rtp_file_num =%d\n", __func__,
...
irqreturn_t aw86927_irq(int irq, void *data)
{
	...
	if ((ret & AW86927_SYSINT_FF_AEI) && aw86927->rtp_init) {
		pr_debug("%s: aw86927 rtp fifo almost empty\n", __func__);
		while ((!aw86927_haptic_rtp_get_fifo_afs(aw86927)) &&
			(aw86927->play_mode == AW86927_RTP_MODE) &&
			!atomic_read(&aw86927->exit_in_rtp_loop)) {
			mutex_lock(&aw86927->rtp_lock);
			if (!aw86927_rtp) {
				pr_info("%s:aw86927_rtp is null, break!\n",
					__func__);
				mutex_unlock(&aw86927->rtp_lock);
				break;
			}
	...
	if (ret & AW86927_SYSINT_FF_AFI)
		pr_info("%s: aw86927 rtp mode fifo almost full!\n", __func__);
...
int aw86927_haptic_init(struct aw86927 *aw86927)
{
	...
	aw86927_haptic_swicth_motor_protect_config(aw86927, 0x00, 0x00);
					...
					struct ff_effect *old)
{
	...
	if (aw86927->effect_type == FF_CONSTANT) {
		...
		pr_debug("%s: aw86927->effect_id =%d\n",
		...
		if (aw86927->effect_id < aw86927->info.effect_id_boundary) {
			...
			pr_debug("%s: aw86927->effect_id=%d , aw86927->activate_mode = %d\n",
				__func__, aw86927->effect_id,
				aw86927->activate_mode);
			/*second data*/
